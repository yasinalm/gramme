...
class CameraModel:
    ...
    def undistort(self, image):
        ...
        lut = self.bilinear_lut[:, 1::-
                                1].T.reshape((2, image.shape[0], image.shape[1]))

        if len(image.shape) == 1:
            raise ValueError(
                'Undistortion function only works with multi-channel images')
    ...
    def __load_intrinsics(self):
        intrinsics_path = Path(__file__).parent/(self.model_name + '.txt')
    ...
    def __load_lut(self):
        lut_path = Path(__file__).parent / \
            (self.model_name + '_distortion_lut.bin')
